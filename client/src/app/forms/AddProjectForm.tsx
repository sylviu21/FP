...
interface FormProjectValues
  extends Omit<
    Project,
    'id' | 'date' | 'timeSpent' | 'hours' | 'minutes'
  > {
  name: string;
  description?: string;
  client: string;
...
const AddProjectForm: FC<AddProjectFormProps> = ({ onSubmit }) => {
  ...
  const isFormValid = (): boolean => {
    ...
    console.log(selectedDeadline, today, selectedDeadline < today);
    setErrors(newErrors);
  ...
  const handleSubmit = (event: FormEvent<HTMLFormElement>) => {
    ...
    const newProject: Project = {
      ...
      timeSpent: { hours: 0, minutes: 0 }, // You can set initial timeSpent values if needed
      ...formData,
    };
